...
# * Neda Jahanshad - neda.jahanshad@ini.usc.edu - ENIGMA_DTI 2014.
# * Adapted by Rachel Brouwer - r.m.brouwer-4@umcutrecht.nl to account small differences in MDS components for MZ twins
# * Modified for ENIGMA-DTI-GWAS by Gabriella Blokland, Barbara Molz, Nina Roth Mota
# * Last edit 2024 Feb, by gblokland - gam.blokland@maastrichtuniversity.nl
####################################################################
...
estimateRelatedness_2gen <- function(data, FID=data$FID, IID=data$IID, PID=data$PID, MID=data$MID, sex=data$Sex, MDS1=data$C1, MDS2=data$C2, MDS3=data$C3,MDS4=data$C4, nullVALUE=0){
    ...
    for (i in 1:(numsubjects-1)) {
        ...
        for (j in (i+1):numsubjects) {
			...
			if (sex[i]==sex[j] && ( abs(as.numeric(MDS1[i]) - as.numeric(MDS1[j])) < 0.0001 && abs(as.numeric(MDS2[i]) - as.numeric(MDS2[j])) < 0.0001 && abs(as.numeric(MDS3[i]) - as.numeric(MDS3[j])) < 0.0001 && abs(as.numeric(MDS4[i]) - as.numeric(MDS4[j])) < 0.0001)) {
